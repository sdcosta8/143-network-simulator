...
def plot_each_graph_separate(points_dict, last_time, test_case, protocol):
    for item in points_dict.items():
        graph_dict = {}
        graph_dict[item[0]] = item[1]
        plot_graphs(graph_dict, last_time, 1, test_case + '/' + protocol + 
            item[0].split(' (')[0] + '.png', 1, 0.5, 1.15)

# For each series in each graph, we should plot them separately
def plot_each_series_separate(points_dict, last_time, test_case, protocol):
    for item in points_dict.items():
...
def plot_2_direction(data, last_time, y_label, protocol):
    L1_right = None
    L1_left = None
    L2_right = None
    L2_left = None
    ...
    'test2RightDirection' + protocol + y_label.split(' (')[0] + '.png', 1, 0.5, 1.15)
    plot_graphs({y_label: [L1_left, L2_left, L3_left]}, last_time, 1, 'test2/' + \
    'test2LeftDirection' + protocol + y_label.split(' (')[0] + '.png', 1, 0.5, 1.15)
...
# L1_left and L2_left
# L3_left and L4_left
def plot_parallel_links(data, last_time, y_label, protocol):
    L1_right = None
    L1_left = None
    L2_right = None
    L2_left = None
    ...
    plot_graphs({y_label: [L1_right, L2_right]}, last_time, 1, 'test1/' +  
        protocol + 'Parallel' + "L1L2_right" + y_label.split(' (')[0] + '.png', 1, 0.5, 1.15)
    plot_graphs({y_label: [L3_right, L4_right]}, last_time, 1, 'test1/' + 
        protocol + 'Parallel' + "L3L4_right" + y_label.split(' (')[0] + '.png', 1, 0.5, 1.15)
    plot_graphs({y_label: [L1_left, L2_left]}, last_time, 1, 'test1/' + 
        protocol + 'Parallel' + "L1L2_left" + y_label.split(' (')[0] + '.png', 1, 0.5, 1.15)
...
def create_graphs(buffer_occ_dicts, packet_loss_dicts, last_time, \
    ...
    link_order, flow_order, test_case, protocol):
    points_dict = {}
    ...
    if GRAPHS_TOGETHER:
        plot_graphs(points_dict, last_time, 6, test_case + '/' + protocol + 
            'AllGraphs.png', 0.8, 1.2, 0.8)

    # Each graph in different figure
    if INDIV_GRAPHS:
        plot_each_graph_separate(points_dict, last_time, test_case, protocol)

    # Each series for each graph in a separate figure
    if INDIV_SERIES:
        plot_each_series_separate(points_dict, last_time, test_case, protocol)
